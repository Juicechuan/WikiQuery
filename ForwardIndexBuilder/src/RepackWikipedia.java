...
public class RepackWikipedia extends Configured implements Tool {
		...
		public void configure(JobConf conf) {
			try {
				...
				throw new RuntimeException(
						"Error loading docno mapping data file!");
			}
		}

		@Override
		public void map(LongWritable key, WikipediaPage doc,
				OutputCollector<IntWritable, WikipediaPage> output,
				Reporter reporter) throws IOException {
			reporter.getCounter(Records.TOTAL).increment(1);
			String id = doc.getDocid();
			if (id != null) {
				// We're going to discard pages that aren't in the docno
				// mapping.
	...
	public int run(String[] args) throws Exception {
		...
		options.addOption(OptionBuilder.withArgName("block|record|none")
				.hasArg().withDescription("compression type")
				.create(COMPRESSION_TYPE_OPTION));
		options.addOption(OptionBuilder.withArgName("en|sv|de").hasArg()
				.withDescription("two-letter language code")
		...
		String compressionType = cmdline
				.getOptionValue(COMPRESSION_TYPE_OPTION);

		if (!"block".equals(compressionType)
				&& !"record".equals(compressionType)
				&& !"none".equals(compressionType)) {
			System.err.println("Error: \"" + compressionType
		...
		conf.setJobName(String.format(
				"RepackWikipedia[%s: %s, %s: %s, %s: %s, %s: %s]",
				INPUT_OPTION, inputPath, OUTPUT_OPTION, outputPath,
				COMPRESSION_TYPE_OPTION, compressionType, LANGUAGE_OPTION,
				language));
