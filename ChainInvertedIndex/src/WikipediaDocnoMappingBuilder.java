 ...
 * @author Peter Exner
 */
public class WikipediaDocnoMappingBuilder extends Configured implements Tool,
		DocnoMapping.Builder {
	private static final Logger LOG = Logger
			.getLogger(WikipediaDocnoMappingBuilder.class);
	...
	public static class MyMapper extends MapReduceBase implements
		...
		public void map(LongWritable key, WikipediaPage p,
			...
			reporter.getCounter(PageTypes.TOTAL).increment(1);
			...
			if (p.isRedirect()) {
				reporter.getCounter(PageTypes.REDIRECT).increment(1);
			} else if (p.isEmpty()) {
				reporter.getCounter(PageTypes.EMPTY).increment(1);
			} else if (p.isDisambiguation()) {
				reporter.getCounter(PageTypes.DISAMBIGUATION).increment(1);
			} else if (p.isArticle()) {
				// heuristic: potentially template or stub article
				if (p.getTitle().length() > 0.3 * p.getContent().length()) {
					reporter.getCounter(PageTypes.OTHER).increment(1);
					return;
				}

				reporter.getCounter(PageTypes.ARTICLE).increment(1);

				if (p.isStub()) {
					reporter.getCounter(PageTypes.STUB).increment(1);
				}

				keyOut.set(Integer.parseInt(p.getDocid()));
				output.collect(keyOut, valOut);
			} else {
				reporter.getCounter(PageTypes.NON_ARTICLE).increment(1);
			}
		}
	}
	...
	@Override
	public int build(Path src, Path dest, Configuration conf)
			throws IOException {
		super.setConf(conf);
		try {
	...
	public int run(String[] args) throws Exception {
		...
		options.addOption(OptionBuilder.withArgName("en|sv|de|cs|es|zh|ar|tr")
				.hasArg().withDescription("two-letter language code")
		...
		if (cmdline.hasOption(LANGUAGE_OPTION)) {
			...
			if (language.length() != 2) {
				System.err.println("Error: \"" + language
						+ "\" unknown language!");
				return -1;
			}
		}
		...
		String tmpPath = "tmp-"
				+ WikipediaDocnoMappingBuilder.class.getSimpleName() + "-"
		...
		conf.setJobName(String.format(
				"BuildWikipediaDocnoMapping[%s: %s, %s: %s, %s: %s]",
		...
		long cnt = keepAll ? job.getCounters().findCounter(PageTypes.TOTAL)
				.getValue() : job.getCounters().findCounter(PageTypes.ARTICLE)
				.getValue();
